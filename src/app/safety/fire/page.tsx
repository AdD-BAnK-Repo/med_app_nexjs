...
import { ArrowLeft, Flame, Clock, Download, Printer, CheckCircle, AlertTriangle } from 'lucide-react';
...
export default function FireExtinguisherCheck() {
  ...
  const fetchItems = () => {
    setLoading(true);
    fetch(`/api/safety/fire?month=${month}&year=${year}`)
      .then(res => res.json())
  ...
  const handleExportCSV = () => {
    const headers = ['ลำดับ', 'จุดที่ติดตั้ง', 'รหัสครุภัณฑ์', 'สถานะสายฉีด', 'สถานะสลักล็อค', 'การคว่ำถัง', 'เกจความดัน', 'สถานะรวม', 'ผู้ตรวจสอบ', 'วันที่ตรวจ', 'หมายเหตุ'];
    const csvContent = [
      headers.join(','),
    ...
    link.setAttribute('download', `fire_extinguisher_report_${month}_${year}.csv`);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 print:bg-white print:pb-0">
      <div className="bg-red-600 text-white pt-12 pb-24 px-4 md:px-8 rounded-b-[2rem] shadow-xl relative z-10 print:hidden">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <button onClick={handleExportCSV} className="flex items-center gap-2 bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-xl font-bold transition">
                <Download size={18} /> CSV
              </button>
              <button onClick={handlePrint} className="flex items-center gap-2 bg-white text-red-600 hover:bg-slate-100 px-4 py-2 rounded-xl font-bold transition shadow-sm">
      ...
      <div className="hidden print:block text-center pt-8 mb-4">
        <h1 className="text-2xl font-bold">รายงานตรวจสอบถังดับเพลิง ประจำเดือน {months.find(m => m.value === month)?.label} ปี {year + 543}</h1>
      </div>
          ...
          <div className="flex flex-col md:flex-row items-end md:items-center justify-between gap-6 mb-8 border-b border-slate-100 pb-6">
            <div className="w-full md:w-auto flex gap-4">
              <div className="flex-1">
                ...
                <select value={month} onChange={e => setMonth(Number(e.target.value))} className="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none">
                  {months.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
                </select>
              </div>
              <div className="flex-1">
                <label className="block text-sm font-bold text-slate-500 mb-2">ปี (ค.ศ.)</label>
                <select value={year} onChange={e => setYear(Number(e.target.value))} className="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none">
              ...
              {Array.isArray(items) ? items.map((item, index) => {
                ...
                return (
                  <div key={item.id} className="border border-slate-100 p-5 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center hover:shadow-md transition-all bg-slate-50/50">
                    ...
                    <Link href={`/safety/fire/${item.id}`} className={`w-full md:w-auto text-center px-6 py-3 rounded-xl font-bold transition-colors ${lastCheck ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-red-100 text-red-700 hover:bg-red-600 hover:text-white'}`}>
